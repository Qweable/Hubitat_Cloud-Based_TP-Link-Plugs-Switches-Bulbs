/*	TP Link Cloud Connect Application, Hubitat Version 1
...
def cloudLogin() {
	setInitialStates()
	def cloudLoginText = "If possible, open the IDE and select Logs.  THEN, " +
		"enter your Username and Password for TP-Link (same as Kasa app) and the "+
		"action you want to complete.  Available actions:\n\r" +
...
def parseDevices(currentDevices) {
	state.devices = [:]
	def devices = state.devices
	currentDevices.each {
		def device = [:]
...
def getDeviceData() {
	...
	httpPost(getDevicesParams) {resp ->
		if (resp.status == 200 && resp.data.error_code == 0) {
			currentDevices = resp.data.result.deviceList
			if (state.currentError != null) {
				state.currentError = null
			}
			parseDevices(currentDevices)
		} else if (resp.status != 200) {
			state.currentError = resp.statusLine
			sendEvent(name: "currentError", value: resp.data)
